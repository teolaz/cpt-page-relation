...
//            ), 10000, 2);
		...
		function rewriteCPTArgs( $args, $post_type ) {
			if ( array_key_exists( $post_type, $this->post_page_rel ) ) {
				...
				if (
					! empty( $slug ) && $post_type != 'slides' && ! array_key_exists( 'rewrite', $args ) ||
					! empty( $slug ) && $post_type != 'slides' && $args[ 'rewrite' ] == false ||
					! empty( $slug ) && $post_type != 'slides' && $args[ 'rewrite' ] != false && $args[ 'rewrite' ][ 'slug' ] != $slug
				) {
					$args[ 'rewrite' ] = array(
						'slug'       => $slug,
						'feeds'      => false,
